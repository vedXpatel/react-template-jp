import { useCallback, Children, isValidElement, cloneElement, createElement, useMemo } from 'react';
...
const useItemsWithIds = (children, id = "root") => {
  const normalizeItems = useCallback(
    (items) => {
      ...
      const tabDescriptors = Children.toArray(items).filter(isValidElement).map((child, index) => {
        ...
        const element = child.type === TabPanel ? cloneElement(child, props) : /* @__PURE__ */ createElement(TabPanel, {
          ...props,
          label,
          key: tabId
        }, child);
  ...
  const itemById = useCallback((id2) => sourceMap[id2], [sourceMap]);
